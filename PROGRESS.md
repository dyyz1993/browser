...
> 执行状态：**M0-M11 全部完成 ✅**（含 bug 修复）
> 当前 HEAD: e92454a (snapshot 重生成)
...
## M11 — Bug 修复 ✅ (2 commits)

**Bug 1**: example.com 前面 16 行空白 → 2 行（M11.1, 9b5dfb7 + e92454a）
- 根因：CSS `body{margin:15vh auto}` 的 `vh` 被解析为 `Length::Px(15.0)`
- 修复：vh/vw/vmin/vmax → `Length::Zero`（ASCII 模式无 viewport）
- rem → `Length::Px(value * 16.0)`（标准 root font-size）
- pt → `Length::Px(value * 4/3)`（72pt = 96px 标准）
- 测试：51 css-engine tests 全绿 + snapshot 重生成

**Bug 2**（调查后不是 bug）：demo.html "Loading..." 未被 JS 替换
- 用 `render-file`（不执行 JS）跑 demo.html 当然显示原始 HTML
- 用 `render-script`（执行 JS）跑就正确显示 "Generated by script at runtime..."
- 这是验收方法问题，不是代码 bug
