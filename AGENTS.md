...
| **1:1 像素级渲染** | **不追求和 Chrome 像素级一致**（完整 CSS 引擎+光栅化 = 换项目，见 M80 决策记录）。但 M80 起新增**像素化近似渲染器**（路线 B）：复用现有布局树的像素坐标，把 ASCII 字符网格换成 2D 画布（真实色块/圆角/像素文本），简单站接近 Chrome 观感。ASCII 爬虫路径保持不变 |
