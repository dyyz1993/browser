...
#[tokio::test]
async fn fetch_smart_skips_js_when_ssr_sufficient() {
    // SSR 内容充足（>500 字符）→ --smart 应跳过 JS，快速返回。
    let ssr_html = r#"<!doctype html>
<html><head><title>SSR Page</title></head>
<body>
<article>
<h1>Server Rendered Article</h1>
<p>This is a sufficiently long server-rendered article that should exceed
the smart mode threshold of 500 characters. Lorem ipsum dolor sit amet,
...
nulla pariatur. Excepteur sint occaecat cupidatat non proident.</p>
    ...
    Mock::given(method("GET"))
        .and(path("/ssr"))
        .respond_with(ResponseTemplate::new(200).set_body_string(ssr_html))
        .mount(&server)
        .await;
    let url = format!("{}/ssr", server.uri());

    bin()
        .args(["fetch", &url, "--format", "markdown", "--smart"])
        .assert()
        .success()
        .stdout(predicate::str::contains("Server Rendered Article"))
        // smart 模式 SSR 够时应提示跳过 JS
        .stderr(predicate::str::contains("[smart] SSR content sufficient"));
}

#[tokio::test]
async fn fetch_smart_falls_back_to_js_when_ssr_sparse() {
    // SSR 内容稀疏（<500 字符）→ --smart 应回退跑 JS。
    let sparse_html = r#"<!doctype html>
<html><head><title>Sparse</title></head>
<body>
<div id="app"></div>
<script>__setBody("dynamic content from JS fallback")</script>
</body></html>"#;
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/sparse"))
        .respond_with(ResponseTemplate::new(200).set_body_string(sparse_html))
        .mount(&server)
        .await;
    let url = format!("{}/sparse", server.uri());

    bin()
        .args(["fetch", &url, "--format", "text", "--smart"])
        .assert()
        .success()
        .stdout(predicate::str::contains("dynamic content from JS fallback"))
        .stderr(predicate::str::contains("[smart] SSR too sparse"));
}
