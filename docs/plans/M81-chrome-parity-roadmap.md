...
- [ ] A1. CDP Playwright 双连接支持（server 并发会话，M42 单会话限制解除）
- [ ] A2. hover 事件（mouseenter/mouseleave/mouseover/mouseout 合成 + CLI --hover）
- [ ] A3. focus/blur 事件（--focus 参数；已部分有 activeElement 追踪）
