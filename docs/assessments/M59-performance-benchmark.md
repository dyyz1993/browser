...
> **browser fetch 在静态/CSR 站点上，内存仅为 Chromium 系爬虫的 1/20～1/40，
> 速度相当或更快，且单文件 13MB 零依赖部署。** 重 JS 站点用 `--no-js` 兜底
> 可获得 20 倍加速。核心代价：极重 SPA（纯 CSR 无 SSR 兜底）渲染不如真 Chrome。
...
> **「13MB 单文件、14MB 内存的 SPA 爬虫。静态/CSR 站比 Chromium 系省 20 倍内存，
> 重 JS 站一行 `--no-js` 兜底。scp 上服务器，当 curl 用。」**
