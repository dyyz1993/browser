...
for site in "${SITES[@]}"; do
  ...
  serve_ms=$(python3 -c "
import json
d=json.loads('''$serve_json''')
t=d.get('_timing',{})
print(t.get('total_ms', int(($serve_end-$serve_start)*1000)))
" 2>/dev/null || echo "?")
  serve_content=$(echo "$serve_json" | python3 -c "import json,sys;d=json.load(sys.stdin);print(d.get('content',''))" 2>/dev/null || echo "")
  serve_chars=$(echo -n "$serve_content" | wc -c | tr -d ' ')
  echo "$serve_content" > "$OUT_DIR/serve_${name}.md"
  ...
  # ── 3. 内容完整性对比 ──
  if [ -f "$OUT_DIR/chrome_${name}.html" ] && [ -s "$OUT_DIR/chrome_${name}.html" ]; then
    completeness=$(python3 "$SCRIPT_DIR/completeness.py" \
      --ours "$OUT_DIR/serve_${name}.md" \
...
echo "    serve_ch  = serve 提取的 markdown 字节数"
